import {Clan} from './../models/clan.js';
import {Location} from './../models/location.js';

const token = "Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzUxMiIsImtpZCI6IjI4YTMxOGY3LTAwMDAtYTFlYi03ZmExLTJjNzQzM2M2Y2NhNSJ9..odQwAhsh5wxzKsxnDHIvGSCD5LcG50-fRZl3qQgBjzHQw31WM1rX_f9slSwcOmxCjeuFZ4MgKAt0MK93LQh3yw";


export async function getAllLocations() {

    let objLocations = {
        MethodName: 'sendAPI',
        params: {
            url: "https://api.clashroyale.com/v1/locations",
            token: token
        }
    }

    const response = await fetch("https://esliceu.codiblau.com/clashRoyale.php", {
        method: 'POST',
        body: JSON.stringify(objLocations),
        headers: {
            "Content-Type": "application/x-www-form-urlencoded"
        }
    });

    let finalResponseJson = await response.json();

    let locations = finalResponseJson.items.map(function (location) {
        return new Location(location.id, location.name, location.isCountry);
    });

    return locations;

}

